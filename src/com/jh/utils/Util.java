...
public class Util {
    ...
    public static String DataFormat(Field filed, Object data) throws ParseException {
        ...
        if (filed.getType().equals(Boolean.class)
            ...
            if (data instanceof String) return (String) data;
            SimpleDateFormat formatSecond = new SimpleDateFormat("yyyy-MM-dd");
            return formatSecond.format(data);
        }
        return data.toString();
    }
     ...
     * @throws Exception
     */
    public static boolean ExportExcelFile(Object returnList, Class tigerClass,
                                          HttpServletResponse paramHttpServletResponse, ExportExcel exportExcel)
            throws Exception {
        ...
        if (ExcelInfo.class.isAssignableFrom(tigerClass) && !Cost.EXCEL_NO_DEFINE_FILENAME.equals(exportExcel.fileName())) {
            ExcelInfo excelInfo = (ExcelInfo) tigerClass.newInstance();
            fileName = excelInfo.fileName();
        } else {
            fileName = exportExcel.fileName();
        }
        String[] arrayList = exportExcel.titleList();
        int[] dateType = exportExcel.dataType();
        List<String> titleList = new ArrayList<String>(arrayList.length);
        for (String str : arrayList)
            titleList.add(str);
        //如果没有定义列的值类型，缺省为String类型
        if (dateType.length != titleList.size())
            dateType = new int[titleList.size()];

        //要导出的字段
        String[] fieldList = exportExcel.fieldList();

        //如果是集合类型
        List<Object[]> dataList = new ArrayList<Object[]>();
        if (Collection.class.isAssignableFrom(returnList.getClass())) {
            for (Object obj : (Collection) returnList) {
                String[] temp = new String[fieldList.length];
                int option = 0;
                for (String field : fieldList) {
                    temp[option] = convernData(ReflectionUtil.getField(tigerClass, field),
                            ReflectionUtil.getValue(obj, tigerClass, field));
                    option++;
                }
                dataList.add(temp);
            }
            ...
            for (String field : fieldList) {
                temp[option] = convernData(ReflectionUtil.getField(tigerClass, field),
                        ReflectionUtil.getValue(returnList, tigerClass, field));
                option++;
            }
            dataList.add(temp);
        }
    ...
    public static Class getClass(Object object) throws ClassNotFoundException {
        if (Collection.class.isAssignableFrom(object.getClass())) {
            for (Type result : ((ParameterizedType) object).getActualTypeArguments()) {
                return Class.forName(result.toString().substring(result.toString().indexOf(" ") + 1));
            }
        } else {
    ...
    public static Object[] sortArray(Object[] array, SortInteface sortInteface) {
        return sortArray(array, sortInteface, OrderBy.ASC);
    }

    public static Object[] sortArray(Object[] array, SortInteface sortInteface, OrderBy orderType) {
        if (array == null || array.length == 1) return array;
        for (int j = 0; j < array.length - 1; j++) {
            for (int i = j; i < array.length - 1 - j; i++) {
                if (OrderBy.ASC.equals(orderType)) {
                    if (!sortInteface.compare(array[i], array[i + 1])) {
